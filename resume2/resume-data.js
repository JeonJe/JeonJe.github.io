window.RESUME_DATA = {
  ...
  "character": "코드 밖에서 문제를 정의해 제품 전략을 전환하는 개발자",
  ...
  "experience": [
    {
      ...
      "period": "23.09 ~ 재직중",
      "achievements": [
        "예약 단위 사람 → 진료 공간 전환, 스케줄 변경 <b>건별 → 1회</b> 처리",
        "만성질환 관리 확장 위해 환자 중심 건강기록 도메인 설계",
        "Redis 분산락 기반 동시 예약 제어, <b>6개월+ 이중 예약 0건</b>",
        "스테이징 배포에 8n 자동화 활용, 주 <b>15~20회</b> 수동 배포 요청 제거"
      ]
    },
    {
      "company": "현대 AutoEver",
  ...
  "projects": [
    {
      ...
      "solutions": [
        ...
        },
        {
          "title": "3. 건강검진 연동",
          "items": [
            "위험도 계산에 필요한 데이터를 직접 입력해야 해서 실제 사용이 막히는 문제 파악. 건강검진 데이터 자동 연동으로 해결.",
            "사전 입력된 위험 인자와 건강검진 연동 데이터 충돌 시 유지/덮어쓰기 선택권을 부여하는 정책 설계."
          ]
        }
      ],
      "results": [
        "2주 내 새 도메인으로 전체 프로토타입 기능 전환 완료",
        "비대면 진료 중심 서비스에서 만성질환 관리 플랫폼으로 확장 가능한 도메인 기반 완성",
        "단순 혈압 기록 앱을 임상 신뢰도를 갖춘 건강 위험 분석 도구로 전환 (ACC/AHA 기반, 의료진 검수)",
        "건강검진 데이터 연동으로 심혈관 위험도 자동 계산까지 원클릭 처리, 기존 수동 입력 건강 위험인자와 충돌 처리 정책 설계 및 구현"
      ],
      "retrospective": [
        "프로토타입으로 빠르게 검증 후 구조 전환, 임상 기준 직접 학습으로 서비스 차별화 기준 수립.",
        "현재 규모에 맞게 심플한 구조로 구현했으나, 사용자 증가 시 캐시 적용, 건강검진 폴링 → 서버 완료 알림 방식 전환 개선 필요."
      ]
    },
    {
      "mode": "deep",
      ...
      "retrospective": [
        ...
        "Redis 장애 시 동시성 제어가 풀리는 한계 인지. 예약 구조 변경 또는 여러 테이블에 걸친 DB 락으로 보완 가능하나, 현재 트래픽과 Redis 가용성을 고려하여 오버엔지니어링으로 판단."
      ],
      "overview": "",
      "team": "백엔드 단독 수행, 프론트엔드 개발자 2명 협업",
      "tasks": [
        "동일 시간대 동시 예약 시 이중 예약 발생 — 환자 대기 발생, 병원 민원 대응 및 수동 예약 변경 비용 발생"
      ],
      "solutions": [
        {
          "title": "시도1. DB락 없는 방식",
          "items": [
            ...
            "낙관적 락/조건부 UPDATE — 버전 체크할 슬롯 Row 없음, 다중 테이블 수동 관리 복잡도 높음",
        ...
        },
        {
          ...
          "items": [
            "비관적 락 — 슬롯 Row 없어 락 대상 부재, 진료실 테이블에 걸면 예약 처리 중 해당 진료실 전체 시간대 블로킹",
            "애플리케이션 락(synchronized) — 단일 JVM 내에서만 유효, K8s 멀티 인스턴스에서 각 JVM이 독립 동작해 인스턴스 간 경쟁 제어 불가"
          ]
        },
        {
          "title": "채택. Redis 분산락",
