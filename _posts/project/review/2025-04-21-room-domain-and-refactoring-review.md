...
## 4. Problem – 아쉬웠던 점
### 1. 프론트엔드 개발자와의 소통
이번 작업은 병원 클라이언트뿐 아니라 환자 앱 프론트엔드 개발자와도 함께 진행해야 했다.
환자 앱은 화면의 변경사항이 없어 작업 범위 계산을 충분히 하지 않았다. 이로 인해 중간에 예상하지 못한 미스 싱크가 발생했고 그로 인해 일부 기능을 다시 개발해야 했다.
어떤 API가 영향을 받고, 어떤 데이터 흐름이 바뀌는지 미리 정리해 공유했더라면 더 원활하게 협업할 수 있었을 것이다.

다음에는 프론트엔드 개발자와 작업 전에 미리 충분히 이야기를 나누어 서로의 싱크를 맞추고, 백엔드 변경 사항은 Swagger 문서를 통해 명확히 공유하는 방식으로 소통을 개선해보고자 한다.

### 2. 일정 지연
이번 프로젝트는 4월 초 배포를 목표로 했지만, 실제 배포는 4월 중순에야 이뤄졌다.
지연의 원인은 개발 도중에 변경된 요구사항들과 충분하지 못했던 테스트 기간이었다.

일정을 연기한 덕분에 스테이징 테스트를 더 오래 진행할 수는 있었지만, 워낙 작업 범위가 큰 프로젝트였기 때문에 배포 후 예상치 못한 부분에서 버그 제보가 들어왔다.
이 버그를 빠르게 수정하면서 오히려 “완벽한 테스트보다는 빠른 배포와 즉각적인 대응이 더 현실적일 수도 있겠다”는 생각이 들었다.
