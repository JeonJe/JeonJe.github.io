...
싱글 스레드 구조에서 하나의 명령어가 오래 걸리면 어떻게 될까?
...
메모리는 데이터를 추가/삭제하다 보면 조각나는 **파편화(fragmentation)** 문제가 있다. 실제 데이터보다 더 많은 메모리를 점유하게 되어, 여유 공간 없이 운영하면 OOM으로 서버가 죽을 수 있다. 그래서 일반적으로 시스템 메모리의 **50~80%** 범위 내에서 관리하는 것이 권장된다.
