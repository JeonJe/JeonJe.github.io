...
| 생산성 도구다 | 강점을 강화하고 약점을 보완하는 동료다 |
| 코드를 빨리 만드는 게 핵심이다 | 무엇을 맡기고 어떻게 검증할지가 더 중요했다 |

AI는 내가 원래 잘하던 것은 더 크게 쓰게 만들었고, 약한 부분은 구조적으로 보완하게 만들었다. 다만 좋은 동료라고 해서 모든 일을 그냥 맡길 수는 없었다. 같이 일하려면 맥락을 줘야 했고, 역할을 나눠야 했고, 결과를 확인할 기준도 필요했다.
...
| 강점 강화 | 반복 문제를 도구로 만들고 싶어 한다 | 반복되는 개발 흐름과 트러블슈팅을 커맨드로 만들어 둔다 |
| 약점 보완 | 시작할 때 무엇을 쓸지 고민이 길다 | 질문을 받아 답하면서 방향을 먼저 정한다 |
...
나중에 보니 이런 방식은 하네스(harness)라고 부르는 것에 가까웠다.  
내게 하네스는 거창한 테스트 자동화라기보다, AI가 그럴듯하게 맞는 척하지 못하게 만드는 검증판이었다. 확인한 것과 모르는 것을 나누고, 증거가 없으면 통과시키지 않는 구조에 가까웠다.

개발 과정도 비슷하게 바뀌었다.

기능 개발이나 버그 수정 하나를 맡으면, 바로 코드로 바꾸게 하지 않는다. 먼저 요구사항과 배경을 정리하고, 설계와 계획을 나눈 뒤, 구현과 리뷰로 이어지는 개발 하네스를 만들었다. AI에게 코드를 빠르게 만들게 하는 것보다, 어디까지 바꿀지와 무엇으로 확인할지를 먼저 정하는 것이 더 중요했기 때문이다.
...
내 약점 중 하나는 시작할 때 고민이 길다는 것이다. 무엇부터 써야 할지 정해지지 않으면 첫 줄이 안 나가고, 그 한 줄 때문에 도중에 포기할 때도 있고 한참이 걸려서야 끝낸 글도 있다.

요즘은 이 부분에서 AI의 도움을 많이 받는다. 내가 원하는 게 무엇인지, 이 글의 목적이 무엇인지 나를 인터뷰하게 한다. 답하다 보면 무엇을 어떻게 쓸지 윤곽이 잡히고, 그 답들을 모아 글을 써 내려간다.
...
AI가 답을 주는 선생님이 아니라, 내가 정말 이해했는지 되묻는 튜터가 되게 한다. 소크라테스식 문답인데, 이 방식이 특히 도움이 많이 됐다. 이 과정을 거치면 내가 안다고 착각했던 부분이 꽤 자주 드러난다.
...
개발하면서 선택은 계속 하지만, 왜 그 선택을 했는지 무엇을 포기했는지는 잘 남기지 않는다. AI는 이 부분을 계속 되묻게 만든다.
...
이런 질문을 반복하다 보면 감각으로만 남아 있던 판단이 문장으로 내려온다. 설계 문서나 회고에서는 어떤 선택을 했는지만큼이나 왜 그렇게 선택했는지를 남기는 게 중요하다.
...
내게 컨텍스트 엔지니어링은 AI가 더 정확하고 근거 있게 판단하도록 맥락을 쌓고 이어주는 일이다.
...
이런 기준들이 쌓이면서 AI 네이티브라는 말도 조금 다르게 보이기 시작했다.

흔히는 AI 도구를 얼마나 잘 다루느냐를 가리키는 말로 쓰인다. 나도 처음에는 어떤 도구를 쓰는지가 먼저 떠올랐다. 하지만 지금은 도구 이름보다, AI가 일할 수 있는 환경을 어떻게 설계하는지가 더 중요하다고 느낀다.

AI를 많이 쓰는 것과 AI가 안전하게 일할 수 있게 만드는 것은 다르다.  
내게 AI 네이티브는 후자에 더 가깝다.

## 마치며

처음에는 AI가 개발자를 대체할 거라는 말이 두려웠다.
지금도 단순히 코드를 빠르게 작성하는 일만 놓고 보면, AI가 많은 부분을 바꾸고 있다.

하지만 내가 직접 써보며 느낀 변화는 조금 달랐다.

AI는 나를 지우기보다, 내가 어떤 사람인지 더 선명하게 만들었다.  
납득하고 검증해야 움직이는 성향은 더 넓게 쓰게 했고, 자꾸 만지작거리며 도구로 만들고 싶어 하는 성향은 더 자주 실행하게 했다. 반대로 시작할 때 고민이 길어지는 것, 이해가 시간이 지나면 흐려지는 것, 선택의 이유를 남기지 못하는 것은 구조적으로 보완하게 만들었다.

그래서 지금은 AI를 얼마나 많이 쓰는지보다, 어디에 둘지가 더 중요하다고 생각한다.

AI를 잘 쓴다는 건 모든 일을 맡기는 것이 아니었다.  
내가 잘하는 것은 더 크게 쓰고, 약한 부분은 구조로 보완할 수 있게 활용하는 일이었다.

코드 타이핑 자체는 줄었지만, 트레이드오프를 따지고 비즈니스 관점에서 문제를 보는 습관이 생겼고 그 문턱도 낮아졌다.
