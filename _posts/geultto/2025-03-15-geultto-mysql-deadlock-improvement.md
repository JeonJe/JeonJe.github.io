...
정기 배포 후, 데이터베이스에서 데드락이 발생했다는 경고 알림이 떴다.
갑작스러운 데드락 알림에 당황했던 기억이 생생하다. 이번 글에서는 갑작스럽게 발생한 데드락 원인을 분석하고, 해결해 나간 경험을 공유하고자 한다.

### 이 글에서 다루는 내용

이번 글에서는 MySQL 저장 프로시저에서 발생한 데드락 문제를 분석하고, 어플리케이션 로직으로 전환하면서 트랜잭션 처리 시간을 약 1400ms → 200ms로 단축한 과정을 다루었다.

- 데드락 발생 원인: `GROUP BY` 사용 시 정렬 순서 미지정으로 인한 트랜잭션 잠금 충돌
- 해결 방안 탐색: 3가지 해결 방식 비교 - `ORDER BY` 추가, `@Transactional` 조정, 어플리케이션 로직 전환
...
이 에러 메시지는 MySQL에서 데드락이 감지되어 트랜잭션이 롤백되었다는 뜻이다. 관련 로그를 추가로 확인한 결과, 환자가 앱에 로그인 후 병원별 마케팅 동의 이력을 업데이트하는 과정에서 데드락이 발생했음을 확인했다.
``

### 2. 데드락 로그 확인

데드락이 발생한 정확한 원인을 파악하기 위해 MySQL InnoDB 엔진 상태를 조회하는 `SHOW ENGINE INNODB STATUS` 명령어를 실행해보았다.
하지만 최근 데드락 정보가 남아있지 않았다. 때문에 정확한 이유는 찾지 못했지만, 다음과 같은 이유로 데드락 기록이 남지 않은 것으로 추정하였다.
...
즉, 데드락 발생 당시 다음과 같은 과정으로 인해 최근 데드락 로그가 남지 않은 것으로 추측된다:

1. InnoDB가 데드락을 감지하고 자동으로 롤백하여 알림이 발생했다
2. 재시도가 진행되어 트랜잭션이 성공적으로 완료되었다
3. `SHOW ENGINE INNODB STATUS`에 최근 데드락 기록이 남지 않았다

> 데드락 감지 기능은 편리하지만 단점도 있다. 동시에 많은 쓰레드가 처리되거나 트랜잭션이 많은 잠금을 가지고 있을 때는 성능에 부담을 줄 수 있다. 이런 경우 `innodb_deadlock_detect=OFF`로 끄고 `innodb_lock_wait_timeout` 값을 설정해서 일정 시간 내에 잠금을 획득하지 못하면 에러를 반환하게 할 수 있다. 단, 기본 타임아웃 값인 50초는 너무 길어서 더 짧은 시간으로 설정하는 것이 좋다.

#### 데드락 모니터링 개선하기

데드락 발생 당시 `innodb_print_all_deadlocks` 설정은 비활성화되어 있어 잠금이 꼬인 정확한 순서와 원인을 파악하기 어려웠다. 이로 인해 문제 분석에 많은 시간을 소모했다.

`innodb_print_all_deadlocks`를 활성화하면 MySQL 에러 로그(`/var/log/mysql/error.log`)에 데드락 발생 시점의 트랜잭션 정보, 잠금 상태, SQL 문 등 상세 정보가 기록된다. 이번 데드락 발생을 계기로 개발 및 운영 환경의 데드락 로깅 관련 설정을 점검하고 개선했다.
...
데드락을 자세히 파악하기 위해 위와 같이 로그 설정을 하는 것도 좋지만, 주의도 필요하다. 데드락이 자주 발생하면 에러 로그 파일이 빠르게 커질 수 있고, 일반 정보 메시지까지 모두 기록되면 정작 중요한 데드락 정보를 찾기 어려워질 수 있다. 운영 환경에서는 이런 점을 고려한 설정이 필요하다

---

## 데드락 원인 분석

데드락이 발생한 프로시저의 구조를 자세히 살펴보았다.

### 기능 개요

먼저 데드락이 발생한 기능이 무엇을 하는지 간단히 설명하면:
...
코드에서 `GROUP BY 병원식별자, 병원환자의 계정식별자`가 데드락의 원인이 되었다. 왜 이 부분이 문제가 되었을까?
...
1. 커서 기반 처리를 배치 처리로 전환하여 큰 성능 향상이 기대되었다
...
1. **필터링**: `WHERE 병원환자식별자 IN (...)` - 회원 계정과 관련된 가족 환자 식별자로 먼저 필터링한다. 약관동의 테이블에 병원환자 식별자가 인덱스로 설정되어 있어 빠른 조회가 가능하다
...
기존 프로시저에서 계산하던 마케팅 약관 동의 상태(Y/N)는 어플리케이션에서 계산하도록 변경하여 책임을 나누고 쿼리의 복잡도를 낮추었다. 개선 결과 개발 데이터 기준 **데이터 스캔량을 30만 건 → 4건으로 감소**, 쿼리 실행 시간 **102.6ms → 0.88ms로 99% 개선**할 수 있었다.
...
한 번에 처리하는 데이터가 최대 100건 미만이어서 별도로 `배치 사이즈`를 설정하지 않았다.
...
프로시저를 어플리케이션 로직으로 전환한 결과는 유의미했다. 트랜잭션 처리 시간이 1,395ms에서 204ms로 85% 단축됐고, 개선 후 한 달 이상 모니터링했지만 더 이상 데드락이 발생하지 않았다.
...
이런 질문을 스스로 던지고 답을 찾아가는 과정에서 MySQL의 트랜잭션과 잠금 과정에 대해 더 깊이 이해하게 됐다. 또한 `innodb_deadlock_detect`와 `innodb_print_all_deadlocks` 같은 설정이 운영 환경에서 문제를 해결하는 데 얼마나 중요한지 직접 체감했다. 문제가 발생했을 때 원인을 빠르게 파악할 수 있는 로깅 설정과 모니터링 체계가 필수적이라는 점을 다시 한번 깨달았다.
