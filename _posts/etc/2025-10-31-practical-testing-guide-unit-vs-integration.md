...
이번 글은 우선 스스로에게, 그리고 이제 막 테스트를 배우고 적용해보려는 개발자분들에게
**왜 테스트를 해야 하는지**, **좋은 테스트는 무엇인지**, **단위 테스트와 통합 테스트는 어떻게 다른지**를 정리해 보려고 한다.
...
```java
// Stub 예시
when(userRepository.findById(1L)).thenReturn(new User("홍길동"));

// Mock 예시
orderService.createOrder(user, product);
verify(emailService, times(1)).sendOrderConfirmation(user.getEmail());
