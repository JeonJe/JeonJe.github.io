...
필터의 위치는 Dispatcher Servlet에 도달하기 전과 Dispatcher Servlet에서 나온 Response에 대해 적용 될 수 있습니다.

필터는 스프링 컨텍스트인 Dispatcher Servlet, Interceptor, Controller가 아니고, 달리 톰캣과 같은 WAS내의 ApplicationContext에서 등록된 필터가 되는 것입니다. 즉, J2EE 표준스펙인 Filter는 Spring Framework가 아니여도 사용이 가능합니다.

### 필터 메소드
필터를 사용하기 위해서는 javax.servlet의 Filter 인터페이스를 구현해야하며 아래와 같은 메소드를 가집니다.
```java
...
사용자가 설정한 url 패턴에 대응되는 HTTP 요청이 Dispatcher Servlet에 전달되기 전에 웹 컨테이너에 의해
실행되는 메소드입니다. 

파라미터인 FilterChain의 doFilter 메소드를 통해 다음 필터로 요청을 전달 할 수 있으며 필터에서 필요한 작업은 chain.doFilter 전/후에 작성합니다.
...
인터셉터를 사용하기 위해서 org.springframework.web.servlet의 HandlerInterceptor 인터페이스를 구현해야합니다.
```java
...
컨트롤러가 호출되기 전에 전처리 작업 / Request 정보를 바꾸거나 추가할 때 사용합니다.

**postHandle**

컨트롤러가 호출된 후, View가 렌더링 되기 전에 후처리 작업을 위해 사용합니다.
...
```
필터의 경우엔 doFilter의 인자에 다른 Request / Response를 넣어 다음 체인으로 전달할 수 있습니다.

하지만, 인터셉터의 preHandle의 경우엔 true (컨트롤러로 전달) / false (요청 중단) 으로 반환되기 때문에
